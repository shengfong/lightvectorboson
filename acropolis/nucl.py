...
class NuclearReactor(object):
    ...
    def __init__(self, s0, s1, sc, temp_rg, e0, e1, ii): ###### second "delta" source
        self._sII = ii
        ...
        # The source terms without the delta function
        self._sS0 = s0

        ###### second "delta" source (without the delta function)
        self._sS1 = s1
    ...
    def _pdi_rates(self, T):
        ...
        xsp, ysp = self._sGen.nonuniversal_spectrum(
                            self._sE0, self._sE1, self._sS0, self._sS1, self._sSc, T 
                        )
        ...
        ###### second "delta" source
        rate_photon_E1 = self._sGen.rate_photon(self._sE1, T)
        ...
        for rid in _lrid:
            ...
            ###### second "delta" source
            I_dt1 = self._sS1[0](T)*self.get_cross_section(rid, self._sE1)/rate_photon_E1

            # Add the delta term and save the result
            pdi_rates[rid] = I_dt + I_dt1 + I_Fs[0]   ###### second "delta" source
