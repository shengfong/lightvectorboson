...
@nb.jit(cache=True)
def _JIT_solve_cascade_equation(E_rt, E1id, G, K, S0, S1, Sc, T):
    EC   = me2/(22.*T)
    Ecut = E_EC_cut*EC
    ...
    _JIT_set_spectra(F_rt, -1, np.array([
        Sc[X,-1]/G[X,-1] + np.sum(K[X,:,-1,-1]*S0[:]/(G[:,-1]*G[X,-1])) \
        + np.sum(K[X,:,-1,E1id]*S1[:]/(G[:,E1id]*G[X,-1])) for X in range(NX) ###### second "delta" source
    ]), E_rt[-1] > Ecut)
    
    #print(E_rt[E1id]) #test
    ...
    while i >= 0:
        ...
        for j, X in enumerate( range(NX) ):
            ...
            a0  = K[X,:,i,-1]*S0[:]/G[:,-1] + .5*dy*E_rt[-1]*K[X,:,i,-1]*F_rt[:,-1] \
                  + K[X,:,i,E1id]*S1[:]/G[:,E1id] ###### second "delta" source
...
class SpectrumGenerator(object):
    ...
    def nonuniversal_spectrum(self, E0, E1, S0, S1, Sc, T, allX=False): ###### second "delta" source
        # Define the dimension of the grid
        # as defined in 'params.py'...
        ...
        ###### second "delta" source: locate the entry of closest energy in E_rt
        E1id = np.argmin( np.abs( E1 - E_rt ))
        #print(E_rt[E1id],E1,E0) #test
        ...
        ###### second "delta" source
        S1 = np.array([S(T) for S in S1])

        Sc = np.array([[ScX(E, T) for E in E_rt] for ScX in Sc])

        # Calculate the spectra by solving
        # the cascade equation
        res = _JIT_solve_cascade_equation(E_rt, E1id, G, K, S0, S1, Sc, T) ###### second "delta" source
